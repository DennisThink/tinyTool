...
void CSessManager::start(CSourceServer* pServer,asio::io_service& ioService, std::error_code& ec)
{
	...
	int nSessIndex = 0;
	auto pSess = std::make_shared<CAPNsSess>(ioService,m_config.m_strServerIp,m_config.m_nPort,this);
	if(pSess)
	{
		INFO(ms_loger,"Add Sess To Map");
		m_SessMap.insert(std::pair<int,std::shared_ptr<CAPNsSess>>(nSessIndex,pSess));
	}
	nSessIndex++;
